...
const ANCHOR_PATTERN = /(?:[a-zA-Z]:\/)?[\w./-]+\.(ts|js|tsx|jsx|py|go|rs|java|rb|cs|md|json|yaml|yml|toml|sh):\d+/;
...
Return only valid JSON.${skillsBlock}`;
   ...
   */
  /**
   * Extract file:line citations from evidence text. Prefers full paths inside
   * `<cite tag="file">` tags (Unity/Assets layouts), then falls back to inline
   * path:line patterns in prose.
   */
  private extractCitationsFromEvidence(evidence: string): Array<{ file: string; line: number }> {
    const seen = new Set<string>();
    ...
    // Priority 1: explicit cite tags — preserves repo-relative paths like Assets/.../Foo.cs
    const citeFilePattern = /<cite\s+tag=["']file["']\s*>([^<]+)<\/cite>/gi;
    let citeMatch: RegExpExecArray | null;
    while ((citeMatch = citeFilePattern.exec(evidence)) !== null) {
      const inner = citeMatch[1].trim();
      const m = inner.match(/^(.+?):(\d+)$/);
      if (m) add(m[1].trim(), parseInt(m[2], 10));
    }

    // Priority 2: inline path:line in prose (strip quoted/code regions first)
    ...
    // Capture full repo-relative path when present (Assets/.../File.cs:18), not just basename.
    const citationPattern = /(?:[a-zA-Z]:\/)?([\w./-]+\.[a-z]{1,6}):(\d+)/gi;
    let match: RegExpExecArray | null;
    while ((match = citationPattern.exec(stripped)) !== null) {
      add(match[1], parseInt(match[2], 10));
    }
  ...
  async verifyCitations(evidence: string, opts: { strict?: boolean } = {}): Promise<boolean> {
    ...
    //
    // Regions stripped inside extractCitationsFromEvidence (prose path only):
    ...
    //
    // <cite> tag contents are parsed directly and are never stripped.
    const citations = this.extractCitationsFromEvidence(evidence);
  ...
  private async resolveFilePath(
    ...
    for (const root of roots) {
      ...
      const searchDirs = ['packages', 'src', 'apps', 'Assets', 'docs', 'tests', 'test', 'tools', 'scripts', 'lib'];
  ...
  private async grepIdentifier(identifier: string): Promise<{ file: string; line: number; snippet: string } | null> {
    ...
    const CONTEXT_LINES = 2;
    const searchDirs = ['packages', 'src', 'apps', 'Assets', 'docs', 'tests', 'test', 'tools', 'scripts', 'lib'];
    const sourceExts = new Set(['.ts', '.tsx', '.js', '.jsx', '.cs']);
