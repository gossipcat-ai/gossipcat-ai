...
const log = (agentId, msg) => process.stderr.write(`[worker:${agentId}] ${msg}\n`);
/**
...
function parseTextToolCalls(text, validTools) {
    ...
    while ((match = blockRe.exec(text)) !== null) {
        ...
        if (parsed)
            calls.push({ id: (0, crypto_1.randomUUID)().slice(0, 12), ...parsed });
    }
    ...
    if (calls.length === 0 && validTools.size > 0) {
        ...
        while ((match = funcRe.exec(text)) !== null) {
            if (!validTools.has(match[1]))
                continue; // double-check
            try {
                const args = JSON.parse(match[2].replace(/,\s*([}\]])/g, '$1'));
                calls.push({ id: (0, crypto_1.randomUUID)().slice(0, 12), name: match[1], arguments: args });
            }
