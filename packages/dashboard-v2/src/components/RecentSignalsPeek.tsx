        ...
        <ul className="space-y-1.5">
          {items.map((s, i) => (
            <li key={`${s.timestamp}-${i}`} className="flex items-center gap-2 text-xs">
              ...
              <span className="w-20 shrink-0 font-mono uppercase tracking-wide" style={{ color: 'var(--text)' }}>
                {LABELS[s.signal] ?? s.signal}
              </span>
              <span className="w-24 shrink-0 truncate font-mono" style={{ color: 'var(--text-dim)' }}>{s.agentId}</span>
              <span className="flex-1 truncate font-mono" style={{ color: 'var(--text-dim)' }}>{truncate(s.evidence ?? '', 80)}</span>
            </li>
          ))}
        </ul>
      )}
