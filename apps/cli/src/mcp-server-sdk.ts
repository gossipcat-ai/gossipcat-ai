...
// refactor (issue #405); after wrapping tool registrations in a function they
...
export function createMcpServer(): McpServer {
  ...
  // (import hoisted to module top — see top of file for filterWatchEvents/WATCH_MAX_EVENTS.)
  server.tool(
    'gossip_watch',
...
// NOTE: each session now owns its own McpServer instance. The MCP SDK forbids
// calling Protocol.connect() twice on the same server (throws "Already connected
// to a transport. Call close() before connecting to a new transport, or use a
// separate Protocol instance per connection."), so the HTTP transport — which
// must accept multiple inbound connections from one daemon process — needs a
// fresh server per session. See issue #405.
const httpMcpSessions = new Map<string, { server: McpServer; transport: StreamableHTTPServerTransport; timer: ReturnType<typeof setTimeout> }>();
...
async function startHttpMcpTransport(): Promise<void> {
  ...
  const httpServer = createHttpServer(async (req: IncomingMessage, res: ServerResponse) => {
    ...
    if (req.method === 'POST') {
      // Fresh McpServer per session — see issue #405. The MCP SDK's Protocol
      // class throws "Already connected to a transport" on a second .connect(),
      // so reusing a singleton across multiple HTTP sessions is forbidden.
      ...
      const transport = new StreamableHTTPServerTransport({
        ...
        onsessioninitialized: (sid) => {
          ...
          httpMcpSessions.set(sid, { server: httpServer, transport, timer });
...
// ── Start ─────────────────────────────────────────────────────────────────
async function main() {
  // Stdio transport is single-process / single-connection per node invocation,
