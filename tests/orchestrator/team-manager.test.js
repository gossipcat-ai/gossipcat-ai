...
const team_manager_1 = require("../../packages/orchestrator/src/team-manager");
function mockRegistry(agents = []) {
    const store = new Map();
    agents.forEach(a => store.set(a.id, a));
    return {
        get: (id) => store.get(id),
        getAll: () => Array.from(store.values()),
        register: (c) => store.set(c.id, c),
...
describe('TeamManager', () => {
    ...
    beforeEach(() => {
        ...
        tm = new team_manager_1.TeamManager({ registry, pipeline: {}, projectRoot: '/tmp/test-gossip' });
    });
    ...
    test('proposeRemove blocks removal when active tasks exist', () => {
        const pipeline = { getActiveTasks: (_id) => [{ id: 't1' }, { id: 't2' }] };
        tm = new team_manager_1.TeamManager({ registry, pipeline, projectRoot: '/tmp/test-gossip' });
