...
describe('TaskGraph', () => {
    const testDir = (0, path_1.join)((0, os_1.tmpdir)(), `gossip-taskgraph-test-${Date.now()}`);
    const gossipDir = (0, path_1.join)(testDir, '.gossip');
    const graphPath = (0, path_1.join)(gossipDir, 'task-graph.jsonl');
    beforeEach(() => {
    ...
    describe('write events', () => {
        it('creates JSONL file on first event', () => {
            const graph = new orchestrator_1.TaskGraph(testDir);
            graph.recordCreated('t1', 'gemini-reviewer', 'review code', ['code_review']);
            expect((0, fs_1.existsSync)(graphPath)).toBe(true);
            const lines = (0, fs_1.readFileSync)(graphPath, 'utf-8').trim().split('\n');
        ...
        it('records decomposition event', () => {
            ...
            const event = JSON.parse((0, fs_1.readFileSync)(graphPath, 'utf-8').trim());
    ...
    describe('read and reconstruct', () => {
        ...
        it('persists index and loads on new instance', () => {
            ...
            const graph2 = new orchestrator_1.TaskGraph(testDir);
...
//# sourceMappingURL=task-graph.test.js.map