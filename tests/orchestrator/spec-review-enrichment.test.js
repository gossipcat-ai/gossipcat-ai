...
describe('extractSpecReferences', () => {
    it('detects spec file references in task text', () => {
        ...
        const refs = (0, orchestrator_1.extractSpecReferences)(task);
    ...
    it('extracts implementation file paths from spec content (backtick paths)', () => {
        ...
        const refs = (0, orchestrator_1.extractSpecReferences)('some task', specContent);
...
`;
    ...
    it('returns empty array for tasks with no spec references', () => {
        const refs = (0, orchestrator_1.extractSpecReferences)('Fix the login button color');
        expect(refs).toEqual([]);
    });
    ...
    it('rejects paths with .. segments in spec content', () => {
        ...
        const refs = (0, orchestrator_1.extractSpecReferences)('task', specContent);
        expect(refs).toEqual([]);
    });
...
describe('buildSpecReviewEnrichment', () => {
    it('returns null if no files', () => {
        expect((0, orchestrator_1.buildSpecReviewEnrichment)([])).toBeNull();
    });
    it('builds enrichment block with cross-reference instructions', () => {
        const result = (0, orchestrator_1.buildSpecReviewEnrichment)(['src/auth.ts', 'src/router.ts']);
    ...
    it('lists a single file correctly', () => {
        const result = (0, orchestrator_1.buildSpecReviewEnrichment)(['lib/utils.ts']);
...
describe('assemblePrompt with specReviewContext', () => {
    ...
    it('omits spec review block when not provided', () => {
        const result = (0, orchestrator_1.assemblePrompt)({ context: 'ctx' });
        expect(result).not.toContain('--- SPEC REVIEW ---');
    });
});
//# sourceMappingURL=spec-review-enrichment.test.js.map